...
export class APIService {
  ...
  getItems(resourceName: string, params?: object): Observable<object[]> {
    ...
    if (resourceName && resourceName === 'property') {
      resourceName = 'properties';
    } else if (resourceName && resourceName === 'device') {
      resourceName = 'devices';
    // } else if (resourceName && resourceName === 'contract') {
    //   resourceName = 'contracts';
    } else if (resourceName && resourceName === 'user') {
      resourceName = 'users';
    }

    const url = this.apiURL + '/' + resourceName;
    // + '/limit=' + options['pageSize'] + '/start=/head=';
  ...
  getTransactionBySearch(id: any) {
    return this.http.get(this.apiURL + '/search/' + id)
      .map(response => response.json()['_body'] as object);
  }
