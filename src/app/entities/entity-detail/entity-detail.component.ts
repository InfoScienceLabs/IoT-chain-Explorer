...
export class EntityDetailComponent implements OnInit, OnDestroy {
  ...
  public track: number = window.history.length;

  public revbutton: number;
  ngOnChanges(changes: SimpleChanges) {
    console.log(window.history.length);
    this.isbutton = this.track <= 1 ? false : true
    ++this.track;
    this.revbutton = this.track - 1;
    //console.log("isbuttonvalue",this.isbutton);
    //console.log("track value",this.track-1);
  }
  ...
  ngOnInit() {
    ...
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      //console.log('Data observable ', this.dataObservable)
      this.dataObservable
  ...
  goBack() {
    // window.history.back();
    ++this.revbutton - 1;
    this.isbutton = this.revbutton - this.track - 1 <= 1 ? false : true
    this.location.back();
    //this.location.back();
